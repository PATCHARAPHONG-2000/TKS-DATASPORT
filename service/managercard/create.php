...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_SESSION['id_city']['province'])) {
        ...
        $id_users = $_SESSION['id_city']['area'];
    } else {
        $id_province = 'default_status';
        $id_users = 'default_status';
    }
    ...
    try {
        ...
        if ($check) {
            if (move_uploaded_file($_FILES["image"]["tmp_name"], $fileImage)) {
                ...
                $stmt = $conn->prepare("INSERT INTO personnel (firstname, lastname, status, sector, province, image) 
                VALUES (:firstname, :lastname, :status, :sector, :province, :image )");

                $stmt->bindParam(':province', $id_province, PDO::PARAM_STR);
                $stmt->bindParam(':firstname', $firstname, PDO::PARAM_STR);
                $stmt->bindParam(':lastname', $lastname, PDO::PARAM_STR);
                $stmt->bindParam(':status', $status, PDO::PARAM_STR);
                $stmt->bindParam(':sector', $id_users, PDO::PARAM_STR);
